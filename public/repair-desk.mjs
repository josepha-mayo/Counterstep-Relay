...
export function studyNote(report) {
  ...
  report.attempts.forEach((a,i) => {
    ...
    rows.push('',`### Recorded attempt ${i+1}`, '', code(a.answer), '', `${outcome}. Attempt ${a.attempt}. ${a.assisted ? 'Help was used.' : 'No hint or reveal was recorded.'} ${a.independent ? 'Independent first attempt recorded.' : 'Not counted as an independent first attempt.'}`);
  });
...
export function installRepairDesk({getCurrent,isBusy,isDirty,isReady,submitRevision,getReport,downloadText,onError}) {
  ...
  const $=id=>document.getElementById(id);let seen=null;
  ...
  note.onclick=async()=>{if(!getCurrent() || isBusy() || isDirty())return;try {const report=await getReport();if(report)downloadText('counterstep-study-note.md',studyNote(report))}catch(e){onError(e)}};
  return {update(){const current=getCurrent(),view=current?.view,move=nextMove(view),disabled=!current||isBusy()||isDirty()||!isReady();$('nextTitle').textContent=isDirty()?'Check the edited work first':move.title;$('nextDetail').textContent=isDirty()?'The full workboard has edits that have not been checked. Check my repair before using line repair, practice, or exports.':move.detail;$('goNext').disabled=isBusy()||isDirty()||!isReady();note.disabled=disabled;
